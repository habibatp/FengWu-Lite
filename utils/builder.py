...
class ConfigBuilder(object):
    ...
    def get_model(self, model_params = None):
        ...
        if type == 'mts2d_model':
            ...
            raise NotImplementedError("Fine-tuning mode is not used in FengWu-Lite local version.")
        else:
    ...
    def get_dataset(self, dataset_params, split):

        if dataset_params is None:
            raise ValueError(f"dataset_params is None for split={split}")

        from datasets.era5_npy_f32 import era5_npy_f32

        dataset = era5_npy_f32(
            data_dir=dataset_params["data_dir"],
            split=split,
            input_steps=dataset_params.get("input_steps", 2),
            target_steps=dataset_params.get("target_steps", 4),
            sample_stride=dataset_params.get("sample_stride", 1),
        )
    ...
    def get_dataloader(self, dataset_params=None, split='train', batch_size=None, dataloader_params=None):
        ...
        # 🔥 FIX IMPORTANT
        if dataset_params is None:
            dataset_params = self.dataset_params.get(split)

        if dataset_params is None:
            raise ValueError(f"dataset_params is still None for split={split}")
